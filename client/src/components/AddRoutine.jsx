...
const AddRoutine = () => {
    ...
    const handleChangeRoutine = (event) => {
        setRoutineFormState({...routineFormState, [event.target.id]: event.target.value})
    }

    const handleSubmitRoutine = async (event) => {
        event.preventDefault()
        let res = await axios.post(`${BASE_URL}/api/routines/5`, routineFormState) //********/
            ...
            <div className='form-container'>
                <form onSubmit={handleSubmitRoutine}>
                    <input id="content" 
                        ...
                        placeholder="content"/>
                    <select id="category" onChange={handleChangeRoutine} value={routineFormState.category}>
